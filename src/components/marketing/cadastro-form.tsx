...
export function CadastroForm({ plano }: CadastroFormProps) {
  ...
  // Retomada: e-mail já cadastrado numa empresa pendente_cartao (abandonou o
  // checkout) — cadastrar() já disparou o link de acesso (resetPasswordForEmail).
  // emailEnviado guarda o e-mail digitado ANTES do submit: o FormData é local a
  // handleSubmit e não sobrevive para o JSX da view de sucesso abaixo.
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setEmailEnviado((formData.get('email') as string)?.trim() ?? '')
  ...
  return (
    <div className="w-full max-w-sm">
      ...
      <div className="rounded-2xl border border-border bg-card p-8 shadow-sm">
        {retomada ? (
          /* Retomada de cadastro abandonado: e-mail de acesso já reenviado —
             substitui o card do formulário (mesmo padrão de assinatura-form.tsx). */
          <div className="flex flex-col items-center gap-3 py-2 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Você já tinha começado esse cadastro. Enviamos um link para{' '}
              <strong className="text-foreground">{emailEnviado}</strong> — é só clicar para definir sua
              senha e continuar direto para o pagamento.
            </p>
          </div>
        ) : (
          <>
