...
**Status:** código construído em 2026-07-16 (orquestração 4 streams: schema+calculadora ANTT, CRUD veículos/motoristas/cotações, OCR de CNH, oferta em `/precos` R$397 + admin). Revisado (code review + security review) e corrigido, incluindo fix crítico de RLS no Storage do bucket de CNH. Autocomplete de cidade (IBGE) em Origem/Destino já ativo em produção. Falta só ativar:

1. **Aplicar a migration `20260716220000_frete_antt_eixos.sql`** (mais recente, ainda não aplicada) — adiciona `eixos` como dimensão real do piso ANTT (ver item 3). As 3 migrations anteriores (`202607161*.sql`) já foram aplicadas em produção.
...
3. **Completar a tabela `frete_antt_coeficientes`** — corrigido em 2026-07-16 pra incluir `eixos` (2,3,4,5,6,7,9) como dimensão real do coeficiente (achado ao comparar com planilha de referência de um parceiro do setor — eixos muda o CCD/CC dentro da mesma tabela/tipo de carga, algo que a v1 do schema não cobria). Hoje só tem **Tabela A completa pra 2 tipos de carga** (Carga Geral + Granel Sólido, todas as 7 faixas de eixo, fonte Portaria SUROC nº 4/2026). Ainda faltam: as Tabelas B/C/D inteiras, e os outros 10 tipos de carga da metodologia oficial (frigorificada, perigosa em suas variações, conteinerizada, neogranel etc.) — transcrever da fonte oficial (`calculadorafrete.antt.gov.br` ou anttlegis.antt.gov.br) antes de anunciar cobertura completa.
4. **Configurar `OPENROUTESERVICE_API_KEY`** (env var, Vercel + `.env.local`) — feature de distância automática (origem→destino, botão fica na cotação, dispara ao sair do campo Destino) já está construída e em produção desde 2026-07-17, mas sem a chave ela falha com erro claro e o campo de distância continua editável manualmente. Cadastro grátis (sem cartão) em `account.heigit.org` — mesmo esquema do Google Vision.
5. **Considerar sugerir "piso + margem"** na cotação (não só o piso cru) — insight direto da planilha do parceiro (ele soma 12% por cima como referência de venda, já que o piso ANTT não inclui lucro/imposto/pedágio).
6. Testar upload de CNH real (ângulos, iluminação, foto de celular) pra calibrar a confiança do parser — só testado contra texto simulado até aqui.
7. Testar o cálculo de distância automática com uma chave OpenRouteService real (só testado até aqui com a geocodificação cidade→coordenada; a chamada real à API nunca rodou por falta de chave) — confirmar que o profile `driving-hgv` retorna rota sensata pra rotas brasileiras reais antes de divulgar a feature pro cliente.

**Decisão de custo (2026-07-16/17):** OCR de CNH e distância de rota construídos com provedores gratuitos (Google Vision + parser próprio; OpenRouteService) em vez de Infosimples/idwall/CAF ou Google Routes API — sem cliente pagante ainda, custo fixo/variável extra não é opção agora. Ver `research/26`, `research/27` (CNH) e `research/28` (distância — inclui comparação de qualidade OpenRouteService vs. Google Routes, sem benchmark técnico definitivo encontrado, mas sem alerta grave contra o gratuito tampouco).

**Contexto:** `research/25-modulo-frete-logistica-transportadora.md` (visão geral e MVP), `research/26-consulta-cnh-api-motorista.md` (por que não existe API só-por-número), `research/27-ocr-cnh-upload-foto.md` (provedores prontos vs. construir), `research/28-distancia-rota-cidades-api.md` (OpenRouteService vs. pagos), specs em `.claude/specs/frete-01` a `frete-04`.
